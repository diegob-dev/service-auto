...
function CarsLoading() {
  return (
    <div
      role="status"
      aria-live="polite"
      aria-label="Caricamento auto"
      className="grid auto-rows-fr grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3"
    >
      <span className="sr-only">Caricamento auto in corso…</span>
      {[0, 1, 2].map((item) => (
        <div
          key={item}
          aria-hidden="true"
...
export const CarsList = ({ cars, isLoading, isError, showFilters = false }: CarsListProps) => {
  ...
  if (isLoading) return <CarsLoading />;
          ...
          {showFilters && (
            <section className="mb-8 rounded-xl border bg-card p-5 shadow-sm" aria-labelledby="public-car-filters-title">
              <div className="flex items-start justify-between gap-3">
                ...
                <Button className="hidden lg:inline-flex" type="button" variant="outline" size="sm" disabled={!hasFilters} onClick={() => setFilters({ ...emptyFilters })}>
                ...
                <Button
                  ...
                  aria-controls="public-car-filter-fields"
              ...
              <div id="public-car-filter-fields" className={`${filtersOpen ? "block" : "hidden"} lg:block`}>
                ...
                <Button className="mt-4 w-full lg:hidden" type="button" variant="outline" size="sm" disabled={!hasFilters} onClick={() => setFilters({ ...emptyFilters })}>
                  <RotateCcw /> Azzera filtri
                </Button>
              </div>
            </section>
